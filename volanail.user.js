...
`));
    ...
    class Thumbnail {
        ...
        doLoad(file) {
            let rv = new Promise((resolve, reject) => {
                try {
                    dry.exts.connection.getFileInfo(file.id, (e, info) => {
                        try {
                            this.addInfos(resolve, reject, e, info);
                        }
                        catch (ex) {
                            reject(ex);
                        }
            ...
            rv.catch(ex => {
                var img = new Image();
                img.src = error;
                this.container.insertBefore(img, this.infos);
                throw ex;
            });
            return rv;
        }
        addInfos(resolve, reject, e, info) {
            ...
            if (info.video_thumb) {
                ...
                let video = $e("video", {src: info.video_thumb});
                video.onloadeddata = () => {
                    this.container.insertBefore(video, this.infos);
                    resolve();
                };
                video.onstalled = () => {
                    this.reject();
    ...
    const prepare_file = dry.exportFunction(file => {
        try {
            ...
            file.dom.vnThumbElement = new Thumbnail(file).container;
            if (active) {
                force_update();
            }
        }
